...
- [x] G9: Regenerating every authority-derived artifact is byte-identical to the committed completion evidence
  ...
  MEASURED: 2026-09-02 all four authority-derived regeneration surfaces run at HEAD, not just the CHECK above. `catalog regenerate --release typescript-7.0.2 --check` → `CATALOG PASS catalogs=9` (exit 0). `completion regenerate --check` → `COMPLETION PROGRAM PASS` (exit 0). `diagnostics regenerate --check` → `DIAGNOSTICS PASS` (exit 0). `ledger rebuild --check` → exit 1, `E_SET_MISMATCH: rejected 248829 obligations`, almost all because no receipt records them at all. This box is checked against `completion regenerate` alone, which is narrower than the gate's own wording; the ledger surface is not byte-identical and cannot be until the receipt set covers the obligation set.
  RECEIPTS: 2026-09-02 `verification/evidence/typescript-7.0.2/{0,23}.jsonl` recaptured from `bamti.evidence/v1` to `v2` at catalog digest b9f62d77 (was b98a2864); both cells observe PASS. Before this, `ledger rebuild --check` aborted on `E_JSON: malformed evidence record` without reading any obligation, which masked the set mismatch above.
