...
pub(super) fn analyze(context: &SemanticRuleContext<'_>, diagnostics: &mut Vec<Diagnostic>) {
    emit(
        ...
        "BAMTS-W045",
        "numeric enum crosses an unchecked number boundary",
        "make the conversion explicit at the enum boundary",
    );
    emit(
        context,
        diagnostics,
        ...
        "BAMTS-W048",
        "indexed access relies on a numeric enum reverse mapping",
        "use a forward member name or an explicit lookup table",
    );
}
