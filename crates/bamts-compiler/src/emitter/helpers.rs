...
mod tests {
    ...
    #[test]
    fn awaiter_pulls_generator_once_and_emits_generator_first() {
        let emitted = emit_helpers(&[HelperKind::Awaiter], &HelperOptions::inline(), None);
        assert_eq!(
            emitted.helpers,
            vec![HelperKind::Awaiter, HelperKind::Generator]
        );
        assert!(emitted.prelude.contains("function __generator"));
        assert!(emitted.prelude.contains("function __awaiter"));
        let generator_at = emitted.prelude.find("function __generator").unwrap();
        let awaiter_at = emitted.prelude.find("function __awaiter").unwrap();
        assert!(
            generator_at < awaiter_at,
            "generator must precede awaiter: {}",
            emitted.prelude
        );

        let again = emit_helpers(
            &[
                HelperKind::Awaiter,
                HelperKind::Generator,
                HelperKind::Awaiter,
            ],
            &HelperOptions::inline(),
            None,
        );
        assert_eq!(again.prelude, emitted.prelude);
        assert_eq!(again.prelude.matches("function __awaiter").count(), 1);
    }
    ...
    fn inline_bodies_start_with_the_catalog_identifier() {
        for kind in HelperKind::all() {
            ...
            assert!(
                source.starts_with("function ") && source.contains(kind.ident()),
                "{}",
                kind.ident()
            );
