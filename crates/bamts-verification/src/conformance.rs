...
use crate::corpus::NODE_VERSION;
#[cfg(test)]
use crate::corpus::{
    ArtifactDirectory, CaseSpec, DigestAlgorithm, ExecutionMode, Provenance, bounded_output,
};
...
pub fn run_suite(
    ...
    let check_context = planned
        ...
        .then(|| {
            ...
            Ok::<crate::check_cells::CheckContext, VerificationError>(
                crate::check_cells::CheckContext {
                    code_map,
                    baseline_groups,
                },
            )
...
fn execute_process_cell(
    ...
    let Some(_index_entry) = snapshot.index.entries.get(&plan.entry.input) else {
        return Ok(CellResult {
            entry_id: plan.entry.id.clone(),
            facet: plan.entry.facet,
            backend: plan.backend,
