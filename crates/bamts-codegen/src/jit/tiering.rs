...
//! It owns no code memory and holds no pointers, so it can never violate the
//! W^X lifecycle in [`crate::jit_memory`]: tier decisions select *which*
//! compiled artifact runs, while every mapping transition stays behind the
//! provider's `Writable -> Executable -> Freed` phases. Cancellation mirrors
//! that one-way discipline: a cancelled unit emits no further decisions, ever.
